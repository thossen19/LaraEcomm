...
<div class="max-w-7xl mx-auto py-6">
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow rounded-lg">
            <form action="{{ route('admin.marketing.campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="target_audience" id="target_audience" required
                                   class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                   value="{{ old('target_audience', $campaign->target_audience) }}" placeholder="e.g., All Customers, New Users">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select name="campaign_type" id="campaign_type" required
                                ...
                                <option value="sms" {{ old('campaign_type', $campaign->campaign_type) == 'sms' ? 'selected' : '' }}>SMS</option>
                                <option value="banner" {{ old('campaign_type', $campaign->campaign_type) == 'banner' ? 'selected' : '' }}>Banner</option>
                                <option value="popup" {{ old('campaign_type', $campaign->campaign_type) == 'popup' ? 'selected' : '' }}>Popup</option>
                            </select>
                            @error('campaign_type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="discount_type" id="discount_type" required
                                    ...
                                    <option value="fixed" {{ old('discount_type', $campaign->discount_type) == 'fixed' ? 'selected' : '' }}>Fixed Amount ({{ $currencySymbol }})</option>
                                </select>
                                @error('discount_type')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div>
                                ...
                                <input type="number" name="discount_value" id="discount_value" step="0.01" min="0" required
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                       value="{{ old('discount_value', $campaign->discount_value) }}" placeholder="e.g., 25 or 500">
                            ...
                            <div>
                                ...
                                <input type="number" name="min_purchase_amount" id="min_purchase_amount" step="0.01" min="0"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                       value="{{ old('min_purchase_amount', $campaign->min_purchase_amount) }}" placeholder="e.g., 1000">
                            ...
                            <div>
                                ...
                                <input type="number" name="max_discount_amount" id="max_discount_amount" step="0.01" min="0"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                       value="{{ old('max_discount_amount', $campaign->max_discount_amount) }}" placeholder="e.g., 2000">
                    ...
                    <div class="border-t pt-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="mt-1 text-xs text-gray-500">JPG, PNG, GIF up to 2MB. Leave empty to keep current image.</p>
                                @if($campaign->image)
                                    <div class="mt-2">
                                        <img src="{{ $campaign->image }}" alt="Current campaign image" class="h-20 w-20 object-cover rounded">
                            ...
                            <div>
                                ...
                                <input type="url" name="link" id="link"
                                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                       value="{{ old('link', $campaign->link) }}" placeholder="https://example.com/campaign">
                                @error('link')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
